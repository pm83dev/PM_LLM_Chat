...
import { findRelevantSymbols, formatSymbolsBlock, loadIndex, WorkspaceIndex } from './indexer';
import { buildEditSystemPrompt, buildFixSystemPrompt, EDIT_FORMAT_INSTRUCTION } from './prompts';
...
async function buildAutoContext(
  ...
  try {
    ...
    const rawContext = await buildContextBlock(document, selectionRange, true, false); // includeRelatedFile=true, diagnostics=false per chat
 ...
 */
async function collectReferencedFiles(request: vscode.ChatRequest): Promise<string> {
  ...
  for (const ref of request.references) {
    ...
    if (activeBase && uri.fsPath.startsWith(activeBase.slice(0, -1))) continue;
...
export async function handleChatRequest(
  ...
  const endpoint = config.get<string>('endpoint', 'http://localhost:9000/v1/chat/completions');
  const model = config.get<string>('model', 'gemma4');
  const systemPrompt = config.get<string>('systemPrompt', 'You are an expert software developer.');
